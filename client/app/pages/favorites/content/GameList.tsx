...
export default function GameList({ rows, viewMoreTo, initialItemId }: Props) {
    ...
    const items = useMemo(() => {
        ...
        const filtered = gameList.filter((item) => {
            if (!q) return true;
            return Object.values(item.names).some((name) => name.toLowerCase().includes(q));
        });

        const key = (item: GameItem) => Object.values(item.names)[0] ?? '';
